...
class AdminFunctions extends BaseFunctions
{
    ...
    public $folderType = array('client', 'brand', 'folder');
    ...
    public function __construct(){
        ...
        if ($this->view=="a_index") {
            ...
            if (isset($this->rep['folder']->ID)) {
                ...
                if ($this->rep['folder']->type=="folder") {
    ...
    protected function filesGetObjByClientName( $name ){
        if ($this->databaseConnection()) {
            $q = $this->db_connection->prepare("
                SELECT 
                    * 
    ...
    protected function filesClientAdd( $params ) {
        ...
        $checkName = $this->filesGetObjByClientName($name);
        ...
        if ($id_client=$this->filesInsertDb($params)) {
            ...
            $paramsAssetsClient['name'] = "Assets Client";
    ...
    protected function filesUploadFile( $params ) {
        ...
        if (!empty($my_file['name'])) {
            ...
            if ($folderObj->type=="folder") {
                ...
                $originalFileName = $my_file['name'];

                $temp = explode(".", $originalFileName);
                $fileExtension = strtolower(end($temp));
                $name = htmlspecialchars(basename($originalFileName, ".".$fileExtension));

                $slug = $this->slugify($name);
                $nrChildren = $this->filesGetFoldersChildrenDirectNumber($id_folder);
                $nrChildren++;
                $slug = $slug . "-".str_pad($nrChildren, 3, "0", STR_PAD_LEFT).".".$fileExtension;
                $f_path = $parent_path."/".$slug;
    ...
    protected function filesArchiveAddFile( $selected_id ) {
        ...
        if (isset($fileObj->ID)) {
            ...
            if ($fileObj->type=="client"||$fileObj->type=="brand"||$fileObj->type=="folder") {
                $filePath = $fileObj->f_path;
                $relativePath = substr($filePath, 8); // 8 = strlen('clienti/')
